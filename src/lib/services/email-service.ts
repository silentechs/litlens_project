...
const FROM_EMAIL = process.env.FROM_EMAIL || "LitLens <noreply@litlens.app>";
...
export interface WelcomeToProjectData {
  email: string;
  userName: string;
  projectTitle: string;
  role: string;
...
export async function sendWelcomeToProject(data: WelcomeToProjectData): Promise<{ success: boolean; error?: string }> {
  const html = `
<!DOCTYPE html>
<html>
<head>
...
<body style="font-family: Georgia, 'Times New Roman', serif; background-color: #f5f5f0; margin: 0; padding: 40px 20px;">
  <div style="max-width: 600px; margin: 0 auto; background: white; border: 1px solid #dcdcc6; box-shadow: 0 4px 30px rgba(26, 51, 32, 0.08);">
    ...
    <div style="padding: 40px;">
      ...
      <p style="margin: 0 0 20px; font-size: 18px; color: #1a3320; line-height: 1.6;">
        You have been added to the systematic review project:
      </p>
  ...
  return sendEmail({
    to: data.email,
    subject: `You've been added to "${data.projectTitle}"`,
    html,
  });
}
...
export async function sendMagicLink(data: MagicLinkData): Promise<{ success: boolean; error?: string }> {
  const html = `
<!DOCTYPE html>
<html>
<head>
...
<body style="font-family: Georgia, 'Times New Roman', serif; background-color: #f5f5f0; margin: 0; padding: 40px 20px;">
  <div style="max-width: 600px; margin: 0 auto; background: white; border: 1px solid #dcdcc6; box-shadow: 0 4px 30px rgba(26, 51, 32, 0.08);">
    ...
    <div style="padding: 40px;">
      ...
      <p style="margin: 0 0 20px; font-size: 18px; color: #1a3320; line-height: 1.6;">
        Click the button below to sign in to your LitLens account:
      </p>
